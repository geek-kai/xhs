...
class AccountManagerApp:
    def __init__(self, master):
        ...
        self.thread_events = {}  # 用于存储每个线程的事件对象
    ...
    def create_thread_widgets(self):
        ...
        # 新增终止线程账号按钮
        tk.Button(self.master, text="终止线程账号", command=self.terminate_account_in_selected_thread).grid(row=15, column=1)  # 新增按钮
    ...
    def start_posting(self):
        ...
        self.threads[posting_thread.ident] = {
            "thread": posting_thread,
            "accounts_info": accounts_info  # 存储账号信息
        }
    ...
    def post_to_accounts(self, accounts_info):
        ...
        self.thread_events[thread_id] = stop_event
        # 初始化当前线程的终止账号集合
        ...
        try:
            while accounts_info:
                if stop_event.is_set():
                        logging.info(f"线程 {thread_id} 被终止。")  # 修复此行
                        break  # 直接返回，结束线程
                next_post_times = []
                ...
                for account_info in accounts_info:
                    ...
                    if account_info['username'] in self.thread_terminated_accounts[thread_id]:
                        self.log_account_activity(account_info['username'], f"线程{thread_id}中,账号{account_info['username']},被主动终止")
                        continue  # 跳过被终止的账号
                    # 检查是否需要跳过
                ...
                if next_post_times:
                    ...
                    # 如果 wait_seconds 小于 120 秒，则将其赋值为 120 秒
                    if wait_seconds < 120:
                        wait_seconds = 120
                    for account_info in accounts_info:
            ...
            self.log_account_activity("系统", f"发布过程中发生错误: {str(e)}")  # 记录错误日志
            self.submit_button.config(state=tk.NORMAL)  # 恢复按钮状态

        # 所有发布完成后（如果需要）可以再次启用按钮
        if thread_id in self.threads:
            del self.threads[thread_id]
            # 从 thread_listbox 中移除对应的线程
            for index in range(self.thread_listbox.size()):
                if str(thread_id) in self.thread_listbox.get(index):
                    self.thread_listbox.delete(index)
                    break
    ...
    def terminate_selected_thread(self):
        ...
        if selection:
            ...
            if thread_id in self.threads:
                # 终止线程中的所有账号
                accounts_info = self.threads[thread_id]["accounts_info"]  # 获取账号信息
                for account_info in accounts_info:
                    self.log_account_activity(account_info['username'], f"账号 {account_info['username']} 在线程 {thread_id} 中已被终止发布。")
                # 终止线程
                # self.threads[thread_id]["thread"].join(timeout=0)  # 假设线程可以被安全终止
                # del self.threads[thread_id]
                # self.thread_listbox.delete(selection[0])
                self.thread_events[thread_id].set()  # 设置事件，唤醒线程
                messagebox.showinfo("信息", f"线程 {thread_id} 将被终止。")

    def terminate_account_in_selected_thread(self):
        """终止选中线程中的账号"""
        selection = self.thread_listbox.curselection()
        if selection:
            selected_item = self.thread_listbox.get(selection[0])
